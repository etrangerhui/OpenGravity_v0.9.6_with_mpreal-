...
/***************************************************************************************/
  ...
  3. Utilisation
	il vous faudras glut ( http://www.opengl.org/developers/documentation/glut/index.html ) et éventuellement glui ( http://www.cs.unc.edu/~rademach/glui/ )

	3.1. Compilation
		(sous windows, vous utilisez le projet MSVC++ 6 et puis vous vous débrouillez avec :p)
	...
	3.2. Options de lancement
		OpenGLavity [options]
			-v --version :
				 affiche le numÚro de version
			-h --help :
				 ce que vous etes en train de lire
			-i --input-file :
				 fichier (.unv) Ó ouvrir (indispensable)
			-o --output-file :
				 fichier (.unv) Ó (Úventuellement) Úcrire
			-t --duration :
				 fixer un temps d'ÚxÚcution limitÚ en secondes
			-n --iterations :
				 fixer un nombre fini d'utÚrations Ó calculer
			-x --glut-interface :
